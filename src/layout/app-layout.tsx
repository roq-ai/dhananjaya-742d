...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<NonprofitInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/nonprofits` : null),
    () => getNonprofits({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Nonprofits',
      path: isTenantUser ? `/nonprofits/view/${data?.[0]?.id}` : '/nonprofits',
      entity: 'nonprofit',
    ...
    { name: 'Donors', path: '/donors', entity: 'donor', service: AccessServiceEnum.PROJECT, icon: FiCreditCard },
